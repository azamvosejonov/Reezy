...
def create_group(
    name: str = Form(...),
    user_id: int = Form(...),  # User ID from form data
    ...
    # Create group
    group = models.Group(name=name, creator_id=user_id, image=image_path, description=description)
    db.add(group)
    db.commit()
    db.refresh(group)
    
    # Add creator as admin
    admin = models.GroupAdmin(group_id=group.id, user_id=user_id)
    db.add(admin)
    
    # Add creator as member
    member = models.GroupMember(group_id=group.id, user_id=user_id)
    db.add(member)
...
def add_member(group_id: int, admin_id: int, user_id: int, db: Session = Depends(get_db)):
    is_admin = db.query(models.GroupAdmin).filter_by(group_id=group_id, user_id=admin_id).first()
    if not is_admin:
...
def remove_member(group_id: int, admin_id: int, user_id: int, db: Session = Depends(get_db)):
    is_admin = db.query(models.GroupAdmin).filter_by(group_id=group_id, user_id=admin_id).first()
    if not is_admin:
